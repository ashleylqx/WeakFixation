...
if NEW:
    ...
    class WildcatPool2dFunction(Function):
        ...
        #         return int(k)
        @staticmethod
        def forward(self, input, kmax, kmin, alpha):
            self.kmax = kmax
            self.kmin = kmin
            self.alpha = alpha
            ...
            # self.intermediate(kmax, kmin, alpha)
        ...
        def backward(self, grad_output):
            ...
            # kmax, kmin, alpha = self.intermediate
            ...
            grad_input = grad_output.new().resize_(
                batch_size, num_channels, n).fill_(0).scatter_(2, self.indices_max, grad_output_max).div_(kmax)
            ...
            if kmin > 0 and self.alpha is not 0:
                ...
                grad_input_min = grad_output.new().resize_(
                    batch_size, num_channels, n).fill_(0).scatter_(2, self.indices_min, grad_output_min).mul_(self.alpha / kmin)
    ...
    class WildcatPool2d(nn.Module):
        def __init__(self, kmax=1, kmin=None, alpha=1):
            ...
            # self.WP2dFunction = WildcatPool2dFunction(self.kmax, self.kmin, self.alpha)
        ...
        def forward(self, input):
            ...
            # return self.WP2dFunction.apply(input)
    ...
    class ClassWisePool(nn.Module):
        def __init__(self, num_maps):
            ...
            self.num_maps = num_maps
            # self.CWPFunction = ClassWisePoolFunction(num_maps=self.num_maps)

        def forward(self, input):
            # return ClassWisePoolFunction(self.num_maps)(input)
            # return ClassWisePoolFunction(num_maps=self.num_maps).apply(input)
            # return self.CWPFunction.apply(input)
    ...
    class ClassWisePoolFunction(Function):
        ...
        def backward(self, grad_output):
            ...
            grad_input = grad_output.view(batch_size, num_outputs, 1, h, w).expand(batch_size, num_outputs,
                                                                                   self.num_maps,
                                                                                   h, w).contiguous()
