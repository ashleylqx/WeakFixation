...
class attention_module_multi_head_RN_cls_sa(torch.nn.Module):
    ...
    def forward(self, bbox, roi_feat):
        ...
        if ATT_RES:
            linear_out = torch.mul(linear_out, linear_out_score_sft.unsqueeze(2).unsqueeze(3)) + linear_out
        else:
            linear_out = torch.mul(linear_out, linear_out_score_sft.unsqueeze(2).unsqueeze(3))
...
class Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa(torch.nn.Module):
    ...
    def forward(self, img, boxes, boxes_nums):  # img size 224
        ...
        att_scores = torch.zeros(img.size(0), 3)  # max & min & argmax att_scores = att_scores.to(img.device)
