...
# Annotations: A numpy array of shape N x 5 where N is the number of objects,
# one represented by each row.
# 5 columns represent the top-left x-coordinate, top-left y-coordinate,
# bottom-right x-coordinate, bottom-right y-coordinate, and the class of the object.
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, map, bboxes):
...
class RandomScale(object):
    ...
    def __call__(self, img, map, bboxes):
        ...
        map=  cv2.resize(map, None, fx = resize_scale_x, fy = resize_scale_y)
        ...
        canvas_map = np.zeros(map_shape, dtype = np.uint8)
        ...
        canvas_map[:y_lim,:x_lim] =  map[:y_lim,:x_lim]
...
class RandomTranslate(object):
    ...
    def __call__(self, img, map, bboxes):
        ...
        canvas_map = np.zeros(map_shape).astype(np.uint8)
        ...
        mask_map = map[max(-corner_y, 0):min(map.shape[0], -corner_y + map_shape[0]), max(-corner_x, 0):min(map.shape[1], -corner_x + map_shape[1])]
        canvas_map[orig_box_cords[0]:orig_box_cords[2], orig_box_cords[1]:orig_box_cords[3]] = mask_map
        map = canvas_map
...
class Translate(object):
    ...
    def __call__(self, img, map, bboxes):
        ...
        mask_map = img[max(-corner_y, 0):min(map.shape[0], -corner_y + map_shape[0]), max(-corner_x, 0):min(map.shape[1], -corner_x + map_shape[1])]
        canvas_map[orig_box_cords[0]:orig_box_cords[2], orig_box_cords[1]:orig_box_cords[3],:] = mask_map
        map = canvas_map
...
class RandomShear(object):
    ...
    def __call__(self, img, map, bboxes):
        ...
        if shear_factor < 0:
            img, map, bboxes = HorizontalFlip()(img, map, bboxes)
        ...
        map = cv2.warpAffine(map, M, (int(nW), map.shape[0]))
...
class Resize(object):
    ...
    def __call__(self, img, map, bboxes):
        ...
        map = letterbox_map(map, self.inp_dim)
...
class Sequence(object):
    ...
    def __call__(self, images, maps, bboxes):
        for i, augmentation in enumerate(self.augmentations):
            images_pre = images.copy()
            maps_pre = maps.copy()
            bboxes_pre = bboxes.copy()
            ...
            if random.random() < prob:
                images, maps, bboxes = augmentation(images_pre, maps_pre, bboxes_pre)

            if bboxes.shape[0] == 0:
                return images_pre, maps_pre, bboxes_pre

        return images, maps, bboxes
...
if __name__ == '__main__':
    ...
    map = cv2.imread("messi.jpg", 0)  # opencv loads images in bgr. the [:,:,::-1] does bgr -> rgb
    ...
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomHorizontalFlip(1)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomScale(0.3, diff=True)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomTranslate(0.3, diff=True)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomRotate(20)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomShear(0.2)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = Resize(608)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    img_, map_, bboxes_ = RandomHSV(100, 100, 100)(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)

    seq = Sequence([RandomHSV(40, 40, 30), RandomHorizontalFlip(), RandomScale(), RandomTranslate(), RandomRotate(10),
                    RandomShear()])
    img_, map_, bboxes_ = seq(img.copy(), map.copy(), bboxes.copy())
    plotted_img = draw_rect(img_, bboxes_)
    f, axarr = plt.subplots(1, 2)
    axarr[0].imshow(plotted_img)
    axarr[1].imshow(map_, cmap='gray', vmin=0, vmax=255)