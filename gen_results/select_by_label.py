...
base_path = 'H:/Codes/WF/Preds/SALICON_train/semantic_map/'
...
for i in range(len(files)):
    if files[i][-4:]=='.jpg':
        ...
        if label.sum()<3:
             ...
             #os.remove(base_path+img_name+'.png')
             #os.remove(base_path+img_name+'_gt.png')
        '''
        line = '%s:'%img_name
        for l in range(len(label)):
            if label[l]==1:
                line = line+'%s\t'%coco_id_name_map[l]
        line = line + '\n'
