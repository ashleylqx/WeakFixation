...
from models_all import Wildcat_WK_hd_gs_compf_cls_att_A, Wildcat_WK_hd_gs_compf_cls_att_A_multiscale, \
...
def train_Wildcat_WK_hd_compf_map_cw(epoch, model, optimizer, logits_loss, info_loss, dataloader, args):
                ...
                if hasattr(model, 'centerbias'):
                    if hasattr(model.centerbias, 'fc1'):
                        if model.centerbias.fc1.weight.grad is not None:
                            writer.add_scalar('Grad_hd/gs_fc1', model.centerbias.fc1.weight.grad.abs().mean().item(),
                                              niter)
                            writer.add_scalar('Grad_hd/gs_fc2', model.centerbias.fc2.weight.grad.abs().mean().item(),
                                              niter)
                        ...
                        if model.centerbias.params.grad is not None:
                            writer.add_scalar('Grad_hd/gs_params', model.centerbias.params.grad.abs().mean().item(),
                                              niter)
    ...
    print("Train [{}]\tAverage cps_loss:{:.4f}"
          "\tAverage h_loss:{:.4f}\tAverage rf_loss:{:.4f}".format(epoch,
                                                                   np.mean(np.array(total_cps_loss)),
                                                                   np.mean(np.array(total_h_loss)),
...
def train_Wildcat_WK_hd_compf_map_cw_gbvs(epoch, model, optimizer, logits_loss, info_loss, dataloader, args):
    ...
    for i, X in enumerate(dataloader):
        ...
        rf_maps = rf_maps - torch.min(torch.min(rf_maps, dim=2, keepdim=True).values, dim=1, keepdim=True).values
        rf_maps = torch.relu(
            rf_maps - torch.mean(rf_maps.view(rf_maps.size(0), -1), dim=-1, keepdim=True).unsqueeze(2))  # for gbvs_thm
...
def train_Wildcat_WK_hd_compf_map_cw_bms(epoch, model, optimizer, logits_loss, info_loss, dataloader, args):
    ...
    for i, X in enumerate(dataloader):
        ...
        # rf_maps = GBVS_R * rf_maps # for gbvs
        rf_maps = torch.relu(
            rf_maps - torch.mean(rf_maps.view(rf_maps.size(0), -1), dim=-1, keepdim=True).unsqueeze(2))  # for bms_thm
...
def train_Wildcat_WK_hd_compf_map_cw_alt_alpha(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader,
                                               args, name_model):
    model.train()
    ...
    if os.path.exists(os.path.join(args.path_out, '{}_epoch{:02d}.pt'.format(name_model, epoch - 1))):
        checkpoint = torch.load(os.path.join(args.path_out, '{}_epoch{:02d}.pt'.format(name_model,
                                                                                       epoch - 1)))  # checkpoint is a dict, containing much info
    ...
    for i, X in enumerate(dataloader):
        ...
        prior_maps = prior_maps - torch.min(torch.min(prior_maps, dim=2, keepdim=True).values, dim=1,
                                            keepdim=True).values
        ...
        if i % tb_log_interval == 0:
            ...
            if torch.cuda.device_count() < 2:
                ...
                if hasattr(model.module, 'centerbias'):
                    if hasattr(model.module.centerbias, 'fc1'):
                        if model.module.centerbias.fc1.weight.grad is not None:
                            writer.add_scalar('Grad_hd/gs_fc1',
                                              model.module.centerbias.fc1.weight.grad.abs().mean().item(), niter)
                            writer.add_scalar('Grad_hd/gs_fc2',
                                              model.module.centerbias.fc2.weight.grad.abs().mean().item(), niter)
                            # writer.add_scalar('Grad_hd/gen_g_feature',
                            #                   model.gen_g_feature.weight.grad.abs().mean().item(), niter)
                    else:
                        if model.module.centerbias.params.grad is not None:
                            writer.add_scalar('Grad_hd/gs_params',
                                              model.module.centerbias.params.grad.abs().mean().item(), niter)
                            # writer.add_scalar('Grad_hd/gen_g_feature', model.gen_g_feature.weight.grad.abs().mean().item(),
                            #                   niter)

                if hasattr(model.module, 'box_head'):
                    writer.add_scalar('Grad_hd/box_head_fc6', model.module.box_head.fc6.weight.grad.abs().mean().item(),
                                      niter)
                    writer.add_scalar('Grad_hd/box_head_fc7', model.module.box_head.fc7.weight.grad.abs().mean().item(),
                                      niter)
...
def eval_Wildcat_WK_hd_compf_salicon_cw(epoch, model, logits_loss, info_loss, dataloader, args):
    ...
    return np.mean(np.array(total_cps_loss)) + np.mean(np.array(total_h_loss)), np.mean(
        np.array(total_map_loss))  # uncomment for hth_2_x
...
def test_Wildcat_WK_hd_compf_cw(model, folder_name, best_model_file, dataloader, args):
    if best_model_file != 'no_training':
        checkpoint = torch.load(os.path.join(args.path_out, 'Models', best_model_file + '.pt'),
...
def train_Wildcat_WK_hd_compf_map_alt_alpha_sa(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader,
                                               args, name_model):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_cw_sa_sp(epoch, model, optimizer, logits_loss, info_loss, dataloader, args):
    ...
    for i, X in enumerate(bar):
        ...
        # rf_maps = rf_maps - rf_maps.min() # do not have this previously
        rf_maps = rf_maps - torch.min(torch.min(rf_maps, dim=2, keepdim=True).values, dim=1, keepdim=True).values
        rf_maps = torch.relu(rf_maps - torch.mean(rf_maps.view(rf_maps.size(0), -1), dim=-1, keepdim=True).unsqueeze(
            2))  # for bms_thm, gbvs_thm
...
def train_Wildcat_WK_hd_compf_map_alt_alpha_sa_sp(epoch, model, model_aux, optimizer, logits_loss, info_loss,
                                                  dataloader, args, name_model):
    model.train()
    ...
    for i, X in enumerate(bar):
        ...
        prior_maps = torch.relu(
            prior_maps - torch.mean(prior_maps.view(prior_maps.size(0), -1), dim=-1, keepdim=True).unsqueeze(
                2))  # for bms_thm
        # prior_maps = GBVS_R * prior_maps # for gbvs
...
def test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp_rank(model, folder_name, best_model_file, dataloader, args, tgt_sizes):
    ...
    for i, X in enumerate(dataloader):
        ...
        # ori_img = scipy.misc.imread(os.path.join(PATH_SALICON, 'images', 'test', img_name[0]+'.jpg')) # height, width, channel
        ori_img = scipy.misc.imread(
            os.path.join(PATH_SALICON, 'images', 'train', img_name[0] + '.jpg'))  # height, width, channel
...
def test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp_rank_rebuttal(model, folder_name, best_model_file, dataloader, args,
                                                               tgt_sizes):
    if best_model_file != 'no_training':
        checkpoint = torch.load(os.path.join(args.path_out, 'Models',
                                             best_model_file + '.pt'))  # checkpoint is a dict, containing much info
    ...
    out_folder_gs = os.path.join(args.path_out, folder_name,
                                 best_model_file + '_gs_v3')  # gaussian bias; cw&bbox weights
    # out_folder_sc = os.path.join(args.path_out, folder_name, best_model_file+'_sc') # attention score
    # out_folder_pd = os.path.join(args.path_out, folder_name, best_model_file+'_pd') # pred score
    ...
    for tidx in range(10):
        ...
        for i, X in enumerate(dataloader):
            ...
            for tgt_s in tgt_sizes:
                ...
                if tgt_s == input_w:
                    pred_logits, pred_maps, obj_masks, att_scores, gaussians = model(img=inputs, boxes=boxes,
                ...
                # print(pred_maps.squeeze(1).size(), HLoss_th()(pred_maps.squeeze(1)).item())
                # print(pred_maps.squeeze(1).size(), HLoss_th()(pred_maps.squeeze(1)).item())
                pred_maps_all += F.interpolate(pred_maps, size=(output_h, output_w), mode='bilinear',
                                               align_corners=True)

            # print(pred_maps_all.squeeze().size())
            np.save(os.path.join(out_folder_gs, img_name[0] + '_params.npy'),
                    gaussians.squeeze().detach().cpu().numpy())
...
def train_Wildcat_WK_hd_compf_map_cw_vib_logits(epoch, model, optimizer, logits_loss, info_loss, dataloader, args):
        ...
        if i % tb_log_interval == 0:
            ...
            if torch.cuda.device_count() < 2:
                ...
                if hasattr(model, 'vib_logits'):
                    writer.add_scalar('Grad_hd/vib_encode0', model.vib_logits.encode[0].weight.grad.abs().mean().item(),
                                      niter)
                    writer.add_scalar('Grad_hd/vib_decode0', model.vib_logits.decode[0].weight.grad.abs().mean().item(),
                                      niter)
                ...
                if hasattr(model.mudule, 'vib_logits'):
                    writer.add_scalar('Grad_hd/vib_encode0',
                                      model.mudule.vib_logits.encode[0].weight.grad.abs().mean().item(), niter)
                    writer.add_scalar('Grad_hd/vib_decode0',
                                      model.mudule.vib_logits.decode[0].weight.grad.abs().mean().item(), niter)
    ...
    print("Train [{}]\tAverage cps_loss:{:.4f}\tAverage vib_loss:{:.4f}"
          ...
          "\tAverage izy_bound:{:.4f}\tAverage izx_bound:{:.4f}".format(epoch,
                                                                        np.mean(np.array(total_cps_loss)),
                                                                        np.mean(np.array(total_vib_loss)),
                                                                        np.mean(np.array(total_h_loss)),
                                                                        np.mean(np.array(total_map_loss)),
                                                                        np.mean(np.array(total_izy_bound)),
                                                                        np.mean(np.array(total_izx_bound))))
            ...
            if torch.cuda.device_count() < 2:
                ...
                if hasattr(model, 'feature_refine'):
                    if model.feature_refine.cell_list[0].conv.weight.grad is not None:
                        writer.add_scalar('Grad_hd/lstm_c0',
                                          model.feature_refine.cell_list[0].conv.weight.grad.abs().mean().item(), niter)
                ...
                if hasattr(model.module, 'feature_refine'):
                    if model.module.feature_refine.cell_list[0].conv.weight.grad is not None:
                        writer.add_scalar('Grad_hd/lstm_c0',
                                          model.module.feature_refine.cell_list[0].conv.weight.grad.abs().mean().item(),
                                          niter)
...
def train_Wildcat_WK_hd_compf_map_alt(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader, args,
                                      name_model):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_alt_alpha(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader,
                                            args, name_model):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_alt_alpha_msl(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader,
                                                args, name_model, tgt_sizes):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_sup_alpha(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader,
                                            args):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_bst(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader, args,
                                      name_model):
    model.train()
...
def train_Wildcat_WK_hd_compf_map_sup_msl(epoch, model, model_aux, optimizer, logits_loss, info_loss, dataloader, args,
                                          tgt_sizes):
    model.train()
...
def main_Wildcat_WK_hd_compf_map(args):
    ...
    if phase == 'train':
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_fdim{}_2_{}_rf{}_hth{}_ms_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
            n_gaussian, normf, FEATURE_DIM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature,
            dilate)  # _gcn_all
        ...
        # # model = Wildcat_WK_sft_gs_compf_cls_att(n_classes=coco_num_classes, kmax=kmax, kmin=kmin, alpha=alpha, num_maps=num_maps,
        # #                    fix_feature=fix_feature, dilate=dilate)
        model = Wildcat_WK_hd_gs_compf_cls_att_A2(n_classes=coco_num_classes, kmax=kmax, kmin=kmin, alpha=alpha,
                                                  num_maps=num_maps,
                                                  fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                  normalize_feature=normf)  #################
        #
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb2_2_{}_rf{}_hth{}_ms_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
            n_gaussian, normf, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature,
            dilate)  # _gcn_all
    ...
    #             break
        ...
        # assert os.path.exists(rf_path)
        ds_train = MS_COCO_ALL_map_full_aug(mode='train', img_h=input_h, img_w=input_w, prior=prior)
        # ds_train = MS_COCO_map_full_aug(mode='train', img_h=input_h, img_w=input_w, prior=prior)
        # ds_train = MS_COCO_map_full_aug(mode='train', img_h=input_h, img_w=input_w)
        # ds_train = ILSVRC_full(mode='train', img_h=input_h, img_w=input_w)
        ...
        for i_epoch in range(args.n_epochs):
            ...
            tmp_eval_loss, map_loss = eval_Wildcat_WK_hd_compf_salicon_cw(i_epoch, model, logits_loss, h_loss,
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_{}_thm_rf{}_hth{}_2_ms4_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            n_gaussian, normf, MAX_BNUM, prior, GBVS_R, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature,
            dilate)  # _gcn_all
        ...
        # args.n_epochs = 5
        for i_epoch in range(args.n_epochs):
            train_Wildcat_WK_hd_compf_map_cw_gbvs(i_epoch, model, optimizer, logits_loss, h_loss, train_dataloader,
                                                  args)
            # train_Wildcat_WK_hd_compf_map_cw_bms(i_epoch, model, optimizer, logits_loss, h_loss, train_dataloader, args)

            tmp_eval_loss, _ = eval_Wildcat_WK_hd_compf_salicon_cw(i_epoch, model, logits_loss, h_loss, eval_dataloader,
                                                                   args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa(n_classes=coco_num_classes, kmax=kmax, kmin=kmin, alpha=alpha,
                                                        num_maps=num_maps,
                                                        fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                        normalize_feature=normf)  #################
        ...
        for i_epoch in range(args.n_epochs):
            ...
            tmp_eval_loss = eval_Wildcat_WK_hd_compf_salicon_cw_sa(i_epoch, model, logits_loss, h_loss, eval_dataloader,
                                                                   args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa_new(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
        ...
        #                                 n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight,FEATURE_DIM,kmax,kmin,alpha,num_maps,fix_feature, dilate) #_gcn_all
        if ATT_RES:
            model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_sa_new_fixf_rres_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
                n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha, num_maps,
                fix_feature, dilate)  # _gcn_all

        else:
            model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_sa_new_fixf_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
                n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha, num_maps,
                fix_feature, dilate)  # _gcn_all

        # if ATT_RES:
        #     model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_sa_ftf_2_rres_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
        #                                 n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight,FEATURE_DIM,kmax,kmin,alpha,num_maps,fix_feature, dilate) #_gcn_all
        #
        # else:
        #     model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_sa_ftf_2_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
        #                                 n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight,FEATURE_DIM,kmax,kmin,alpha,num_maps,fix_feature, dilate) #_gcn_all
        #
        # # previsou one5 is actually one2 ... sad ...
        # model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_rng{}_sgd_8_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
        ...
        for i_epoch in range(args.n_epochs):
            ...
            tmp_eval_loss, map_loss = eval_Wildcat_WK_hd_compf_salicon_cw_sa(i_epoch, model, logits_loss, h_loss,
                                                                             eval_dataloader, args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa_art(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                            alpha=alpha, num_maps=num_maps,
                                                            fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                            normalize_feature=normf)  #################
        ...
        if ATT_RES:
            ...
            model_name = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_{}_thm_rf{}_hth{}_ms4_fdim{}_34_cw_sa_art_fixf_3_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
                n_gaussian, normf, MAX_BNUM, prior, GBVS_R, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha,
                num_maps, fix_feature, dilate)  # _gcn_all

            # model_name = 'resnet101_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_sa_art_fixf_3_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate) # for rf/hth ablation

            # model_name = 'resnet50_wildcat_wk_hd_cbG{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_sa_art_fixf_2_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate)
            # model_name = 'resnet50_wildcat_wk_hd_nobs_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_sa_art_fixf_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate)
            # model_name = 'resnet101_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_noGrid_rf{}_hth{}_ms4_sa_art_fixf_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate)  # _gcn_all
            # model_name = 'resnet101_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_norn_rf{}_hth{}_ms4_sa_art_fixf_2_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate)  # _gcn_all
            # model_name = 'resnet101_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_nopsal_rf{}_hth{}_ms4_sa_art_fixf_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
            #     n_gaussian, normf, MAX_BNUM, prior, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate)  # _gcn_all
        ...
        checkpoint = torch.load(os.path.join(args.path_out, 'Models',
                                             ...
                                             '_hth0.1_2_ms4_kmax1_kminNone_a0.7_M4_fFalse_dlTrue_one3_224_epoch11.pt'),
                                map_location='cuda:0')
        # checkpoint = torch.load(os.path.join(args.path_out, 'Models',
        ...
        if ATT_RES:
            model_name = 'resnet50_wildcat_wk_hd_cbA{}_alt_self_{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_{}_rf{}_hth{}_ms4_fdim{}_34_cw_sa_new_fixf_rres_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224'.format(
                n_gaussian, ALPHA, normf, MAX_BNUM, prior, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha,
                num_maps, fix_feature, dilate)  # _gcn_all
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa_art_sp(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                               alpha=alpha, num_maps=num_maps,
                                                               fix_feature=fix_feature, dilate=dilate, use_grid=False,
                                                               normalize_feature=normf)  # noGrid
        ...
        if ATT_RES:
            ...
            checkpoint = torch.load(os.path.join(args.path_out, 'Models',
                                                 ...
                                                 '_ms4_sa_art_fixf_kmax1_kminNone_a0.7_M4_fFalse_dlTrue_one3_224_epoch00.pt'),
                                    map_location='cuda:0')  # checkpoint is a dict, containing much info
            # checkpoint = torch.load(os.path.join(args.path_out, 'Models',
            #                                      'resnet101_wildcat_wk_hd_cbA16_compf_cls_att_gd_nf4_normTrue_hb_50_aug7_nips08_norn_2_rf0.1_hth0.1'+
            #                                      '_ms4_kmax1_kminNone_a0.7_M4_fFalse_dlTrue_one3_224_epoch04.pt'), map_location='cuda:0')
        ...
        # args.n_epochs = 5
        for i_epoch in range(args.n_epochs):
            train_Wildcat_WK_hd_compf_map_cw_sa_sp(i_epoch, model, optimizer, logits_loss, h_loss, train_dataloader,
                                                   args)

            tmp_eval_loss, map_loss = eval_Wildcat_WK_hd_compf_salicon_cw_sa_sp(i_epoch, model, logits_loss, h_loss,
                                                                                eval_dataloader, args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        for i_epoch in range(args.n_epochs):
            ...
            train_Wildcat_WK_hd_compf_map_alt_alpha_sa_sp(i_epoch, model, model_aux, optimizer, logits_loss, h_loss,
                                                          train_dataloader, args, model_name)
            tmp_eval_loss, map_loss = eval_Wildcat_WK_hd_compf_salicon_cw_sa_sp(i_epoch, model, logits_loss, h_loss,
                                                                                eval_dataloader, args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_vib_m_cwmaps(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                                  alpha=alpha, num_maps=num_maps,
                                                                  fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                                  normalize_feature=normf)  #################
        # # #
        ...
        # args.n_epochs = 5
        for i_epoch in range(args.n_epochs):
            train_Wildcat_WK_hd_compf_map_cw_vib_logits(i_epoch, model, optimizer, logits_loss, h_loss,
                                                        train_dataloader, args)

            tmp_eval_loss = eval_Wildcat_WK_hd_compf_salicon_cw_vib_logits(i_epoch, model, logits_loss, h_loss,
                                                                           eval_dataloader, args)
            # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        for idx in range(2, 10):
            ...
            for i_epoch in range(args.n_epochs):
                ...
                tmp_eval_loss = eval_Wildcat_WK_hd_compf_salicon(i_epoch, model, logits_loss, h_loss, eval_dataloader,
                                                                 args)
                # tmp_eval_salicon_loss = eval_Wildcat_WK_hd_compf_map(i_epoch, model, logits_loss, h_loss, eval_map_dataloader, args)
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_alt3_3_{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_nips08_rf{}_hth{}_ms4_fdim{}_34_lstm_cw_1_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
            n_gaussian, ALPHA, normf, MAX_BNUM, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha, num_maps,
            fix_feature, dilate)  # _gcn_all
        ...
        checkpoint = torch.load(
            os.path.join(path_models, 'resnet101_wildcat_wk_hd_cbA16_compf_cls_att_gd_nf4_normTrue_hb_50_aug7_nips08_' +
        ...
        for i_epoch in range(args.n_epochs):
            train_Wildcat_WK_hd_compf_map_cw_alt_alpha(i_epoch, model, model_aux, optimizer, logits_loss, h_loss,
                                                       train_dataloader, args, model_name)
        ...
        model_aux = Wildcat_WK_hd_gs_compf_cls_att_A4_lstm_cw_multiscale(n_classes=coco_num_classes, kmax=kmax,
                                                                         kmin=kmin, alpha=alpha,
                                                                         num_maps=num_maps,
                                                                         fix_feature=fix_feature, dilate=dilate,
                                                                         use_grid=True,
                                                                         normalize_feature=normf)
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_sup2_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_nips08_rf{}_hth{}_ms_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
            n_gaussian, normf, MAX_BNUM, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature,
            dilate)  # _gcn_all
    ...
    if phase == 'train_sm_aug':
        ...
        model_aux = Wildcat_WK_hd_gs_compf_cls_att_A_multiscale(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                                alpha=alpha,
                                                                num_maps=num_maps,
                                                                fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                                normalize_feature=normf)
        ...
        train_dataloader = DataLoader(ds_train, batch_size=args.batch_size, collate_fn=collate_fn_ilsvrc_map_rn,
                                      # collate_fn_coco_map_rn,
                                      shuffle=True, num_workers=2)

        eval_dataloader = DataLoader(ds_validate, batch_size=args.batch_size, collate_fn=collate_fn_ilsvrc_rn,
                                     # collate_fn_coco_rn,
                                     shuffle=False, num_workers=2)
        ...
        model_name = 'resnet50_wildcat_wk_hd_cbA{}_aug7_compf_cls_att_gd_nf4_norm{}_hb_{}_tgt{}_rf{}_hth{}_ils4_fdim{}_34_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one2_224'.format(
            n_gaussian, normf, MAX_BNUM, ilsvrc_num_tgt_classes, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha,
            num_maps, fix_feature,
            dilate)  # _gcn_all
        ...
        # model = Wildcat_WK_hd_gs_compf_cls_att_A(n_classes=ilsvrc_num_classes, kmax=kmax, kmin=kmin, alpha=alpha, num_maps=num_maps,
        #                  fix_feature=fix_feature, dilate=dilate, use_grid=False, normalize_feature=False) #################
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw(n_classes=ilsvrc_num_tgt_classes, kmax=kmax, kmin=kmin,
                                                     alpha=alpha,
                                                     num_maps=num_maps,
                                                     fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                     normalize_feature=normf)  #################
    ...
    # generate maps
        ...
        for e_num in E_NUM:
            ...
            if ATT_RES:
                ...
                best_model_file = 'resnet50_wildcat_wk_hd_cbA{}_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_gbvs_{}_thm_rf{}_hth{}_ms4_fdim{}_34_cw_sa_art_fixf_2_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224_epoch{:02d}'.format(
                    n_gaussian, normf, MAX_BNUM, GBVS_R, rf_weight, hth_weight, FEATURE_DIM, kmax, kmin, alpha,
                    num_maps, fix_feature, dilate, e_num)  # _gcn_all
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa_art_sp_rank_rebuttal(n_classes=coco_num_classes, kmax=kmax,
                                                                             kmin=kmin,
                                                                             alpha=alpha, num_maps=num_maps,
                                                                             fix_feature=fix_feature, dilate=dilate,
                                                                             use_grid=True, normalize_feature=normf)
        ...
        for e_num in E_NUM:
            ...
            # test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp_rank(model, folder_name, best_model_file, test_dataloader, args, tgt_sizes=tgt_sizes)
            test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp_rank_rebuttal(model, folder_name, best_model_file,
                                                                       test_dataloader, args, tgt_sizes=tgt_sizes)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_sa_art_sp_rank(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                                    alpha=alpha, num_maps=num_maps,
                                                                    fix_feature=fix_feature, dilate=dilate,
                                                                    use_grid=True,
                                                                    normalize_feature=normf)  # for storing all the elements
        ...
        for e_num in E_NUM:
            ...
            # test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp(model, folder_name, best_model_file, test_dataloader, args, tgt_sizes=tgt_sizes)
            test_Wildcat_WK_hd_compf_multiscale_cw_sa_sp_rank(model, folder_name, best_model_file, test_dataloader,
                                                              args, tgt_sizes=tgt_sizes)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_multiscale(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                             alpha=alpha, num_maps=num_maps,
                                                             fix_feature=fix_feature, dilate=dilate, use_grid=True,
                                                             normalize_feature=normf)
        ...
        test_dataloader = DataLoader(ds_test, batch_size=args.batch_size, collate_fn=collate_fn_mit1003_rn,
                                     shuffle=False, num_workers=2)
        test_Wildcat_WK_hd_compf_multiscale(model, folder_name, best_model_file, test_dataloader, args,
                                            tgt_sizes=tgt_sizes)
        ...
        model = Wildcat_WK_hd_gs_compf_cls_att_A4_cw_noobj(n_classes=coco_num_classes, kmax=kmax, kmin=kmin,
                                                           alpha=alpha, num_maps=num_maps,
                                                           fix_feature=fix_feature, dilate=dilate, use_grid=False,
                                                           normalize_feature=normf)  #################
        ...
        for e_num in E_NUM:
            ...
            best_model_file = 'resnet101_wildcat_wk_hd_cbA{}_alt_2_compf_cls_att_gd_nf4_norm{}_hb_{}_aug7_nips08_noobj_rf{}_hth{}_ms4_kmax{}_kmin{}_a{}_M{}_f{}_dl{}_one3_224_epoch{:02d}'.format(
                n_gaussian, normf, MAX_BNUM, rf_weight, hth_weight, kmax, kmin, alpha, num_maps, fix_feature, dilate,
                e_num)  # _gcn_all
            ...
            # test_dataloader = DataLoader(ds_test, batch_size=args.batch_size, collate_fn=collate_fn_mit300_rn,
            #                              shuffle=False, num_workers=2)
            test_Wildcat_WK_hd_compf_multiscale_cw(model, folder_name, best_model_file, test_dataloader, args,
                                                   tgt_sizes=tgt_sizes)
...
def parse_arguments():
    ...
    parser.add_argument("--batch_size", default=24 * torch.cuda.device_count(),
                        # cw 72(26xxx) or larger #56(512) can be larger #52 (1024) # 16 5000M, can up to 32 or 64 for larger dataset
                        type=int,
                        # cw512 *80* ; cw1024 *64*; cw512 one5 *32*; cw512 one0 *32(15553),48*; CW512 448input *24*; cw512_101 *42*
                        help="""Set batch size""")  # cw512 msl *64*
    parser.add_argument("--train-batch", default=36 * torch.cuda.device_count(),
                        # cw 72(26xxx) or larger #56(512) can be larger #52 (1024) # 16 5000M, can up to 32 or 64 for larger dataset
                        type=int,
                        # cw512 *80* ; cw1024 *64*; cw512 one5 *32*; cw512 one0 *32(15553),48*; CW512 448input *24*; cw512_101 *42*
                        help="""Set batch size""")  # cw512 msl *64*
    parser.add_argument("--test-batch", default=20 * torch.cuda.device_count(),
                        # cw 72(26xxx) or larger #56(512) can be larger #52 (1024) # 16 5000M, can up to 32 or 64 for larger dataset
                        type=int,
                        # cw512 *80* ; cw1024 *64*; cw512 one5 *32*; cw512 one0 *32(15553),48*; CW512 448input *24*; cw512_101 *42*
                        help="""Set batch size""")  # cw512 msl *64*
