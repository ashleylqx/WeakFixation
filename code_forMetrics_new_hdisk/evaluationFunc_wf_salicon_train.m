function [ meanMetric, allMetrics, frames] = evaluationFunc_wf_salicon_train( options, metricName )
...
for i = 1:nframe
    ...
    fix_gt_path = strrep(gt_fold,'/images/', '/fixations/');
...
%     I = imread(fixation_path);
    I = load_salicon_fix(fixation_path); 
...
%             load(fixation_path);
        %     I = imread(fixation_path);
            ...
            for k = 1:min(10,length(frames))
                ...
                fix_path = strrep(fx_fold,'/images/', '/fixations/');
...
%                 Ix = imread(fixation_pathx);
                Ix = load_salicon_fix(fixation_pathx);
                Ix = Ix>0;
                Ix = double(Ix);
